...
class CustomfunctionComponent extends Component
{
    ...
    public function generateClientId($siteShort='R',$digit=6)
    {        
        $clientID=$this->createClientId($siteShort,$digit);
        return $clientID;
    }
    public function createClientId($siteShort,$digit)
    {
        $clientID=strtoupper($siteShort.substr(uniqid(rand()),0,$digit));
        $Client=ClassRegistry::init('Client');
        $clientArr=$Client->findByHolderId($clientID);
        if($clientArr)
        {
            return$this->createClientId($siteShort,$digit);
        }
        else
        {
            return $clientID;
