...
class DealsController extends AdminAppController {
    ...
    public function add($id=null)
    {
        ...
        if($id==null){
            $this->set('clientArr',array('Client'=>array('id'=>0,'name'=>'')));
        }
        else{
            $this->set('clientArr',$this->Client->findById($id));   
        }        
        ...
        if ($this->request->is('post'))
        {
            ...
            try
            {
                ...
                if ($this->Deal->save($this->request->data))
                {
                    ...
                    $this->Session->setFlash(__('Your Deal has been saved.'),'flash',array('alert'=>'success'));
                    return $this->redirect(array('controller'=>'plans_payments','action' => 'add',$this->Deal->id));
                }
            }
            catch (Exception $e)
            {
                $this->Session->setFlash(__('Please Select Client Name & Property Name & Flat/Plot Name'),'flash',array('alert'=>'danger'));
            }
        }
    }
    public function edit($id = null)
    {
        ...
        if ($this->request->is(array('post', 'put')))
        {
            ...
            try
            {
                if ($this->Deal->save($this->request->data))
                {
                    $this->Session->setFlash(__('Your Deal has been updated.'),'flash',array('alert'=>'success'));
    ...
    public function deleteall()
    {
        try
        {
            if ($this->request->is('post'))
            {
                ...
                $this->Session->setFlash(__('Your Deal has been deleted.'),'flash',array('alert'=>'success'));
            }        
