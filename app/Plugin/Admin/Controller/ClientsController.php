...
class ClientsController extends AdminAppController {
    ...
    public function add()
    {
        if ($this->request->is('post'))
        {
            ...
            try
            {
                $clientId=$this->CustomFunction->generateClientId($this->siteShort,6);
                $passwordHasher = new SimplePasswordHasher(array('hashType' => 'sha256'));
                $password = rand();
                $this->request->data['Client']['password'] = $passwordHasher->hash($password);
                $this->request->data['Client']['holder_id']=$clientId;
                if ($this->Client->save($this->request->data))
                {
                    ...
                    $this->Session->setFlash(__('Your Client has been saved.'),'flash',array('alert'=>'success'));
    ...
    public function deleteall()
    {
        try
        {
            if ($this->request->is('post'))
            {
                ...
                $this->Session->setFlash(__('Your Client has been deleted.'),'flash',array('alert'=>'success'));
            }        
