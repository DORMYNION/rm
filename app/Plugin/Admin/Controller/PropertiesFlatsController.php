        ...
        if ($this->request->is('post'))
        {
            ...
            try
            {
                ...
                if($flatCount==0)
                {
                    ...
                    {
                        $this->Session->setFlash(__('Flat/Plot has been saved.'),'flash',array('alert'=>'success'));
                        return $this->redirect(array('action' => "add/$propertyId"));
                    }
                }
                else
                {
                    $this->Session->setFlash(__('Flat/Plot Name has been already exist'),'flash',array('alert'=>'danger'));
                }
            }
            catch (Exception $e)
            {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(array('post', 'put')))
        {
            ...
            try
            {
                ...
                {
                    $this->Session->setFlash(__('Flats/Posts has been saved.'),'flash',array('alert'=>'success'));
    ...
    public function deleteall()
    {
        try
        {
            if ($this->request->is('post'))
            {
                ...
                $this->Session->setFlash(__('Flats/Plots has been deleted.'),'flash',array('alert'=>'success'));
                $propertyId=$this->data['propertyId'];
            }
